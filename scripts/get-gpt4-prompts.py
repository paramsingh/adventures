...
The genre of the story should be {random.choice(GENRES)}. Try to set the story in India if possible.
...
def git_commit_push(repo_dir, commit_message):
    os.chdir(repo_dir)
    subprocess.run(["git", "add", "responses.json"])
    subprocess.run(["git", "commit", "-m", commit_message])
    subprocess.run(["git", "push"])


def main():
    with yaspin(text="Querying GPT-4..."):
        response = query_gpt(PROMPT)
    print("GPT-4 response:", response)

    write_to_json(json_file_path, response)
    git_commit_push(
        repo_dir,
